...
public class StudentManager {
	...
	private List<Student> studentList; //저장매체
	
	public StudentManager() {
		studentList = new ArrayList<Student>();//포함관계
	}
	
	//학번을 전달받아 콜렉션 필드에 저장된 요소를 검색하여 위치값(첨자)을 반환하는 메소드
	//=>해당 학번의 학생정보가 콜렉션 필드에 저장된 요소가 아닌 경우 -1 반환
	//=>클래스 내부에서만 접근하여 사용하도록 은닉화 처리
	private int getStudentIndex(int num){
		int index = -1;
		
		//콜렉션 필드에 저장된 요소에 대한 반복 처리
		for(int i=0;i<studentList.size(); i++) {
			//콜렉션 필드의 요소(학생정보)의 학번과 전달된 학번을 비교하여 같은 경우
	...
	//학생정보를 전달받아 콜렉션 필드의 요소로 저장하고 저장 결과를 반환하는 메소드
	//=> false: 미저장, true: 저장
	...
	//학생정보를 전달받아 콜렉션 필드의 요소를 변경하고 변경 결과를 반환하는 메서드
	//=> false: 미변경, true: 변경
	//전달받은 학생정보의 학번이 콜렉션 필드에 저장된 요소가 아닌경우 변경되지 않도록 처리
	...
	//전달받은 학번이 콜렉션 필드에 저장된 요소가 아닌경우 null 반환
