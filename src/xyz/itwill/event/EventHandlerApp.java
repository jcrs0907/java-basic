...
//Listener 인터페이스: 이벤트를 처리하기 위한 추상메서드가 선언된 인터페이스
//추상 메소드를 오버라이드 선언하여 이벤트 처리 명령을 작성 - 이벤트 핸들러 메소드
//3.이벤트 소스에서 이벤트가 발생될 경우 이벤트 핸들러 클래스가 동작되도록 이벤트 등록
//Component.addXXXListener(Listener l): 컴퍼넌트에서 이벤트가 발생될 경우
//이벤트 핸들러 클래스로 생성된 인스턴스를 전달받아 이벤트 처리되도록 등록하는 메소드
//이벤트 핸들러 메소드는 자동 호출되어 이벤트 처리

//디자인 클래스와 이벤트 핸들러 클래스를 따로 선언한 경우
//=> 이벤트 핸들러 메소드에서 디자인 클래스의 컴퍼넌트 또는컨테이너 제어 불가능
public class EventHandlerApp extends Frame{
	...
	public EventHandlerApp(String title) {
		...
		//Button 컴퍼넌트를 사용자가 누를 경우 ActionEvent 발생
...
class ActionEventHandler implements ActionListener{
	...
	//ActionEvent가 발생된 경우 자동 호출되는 이벤트 핸들러 메서드
	@Override
